...
async def check_censor(bot, message):
    ...
    #     word_split = word.split("/")
    #     checking_bad_word = word_split[0]
    #     if checking_bad_word in msg_lower_split:
    #         bad_words.append(checking_bad_word)
    #         bad_word_map = [checking_bad_word]

    #         if len(word_split) > 1:
    #             bad_word_map.append(word_split[1])
    #         else:
    #             bad_word_map.append("[redacted]")
    #         if len(word_split) > 2:
    #             bad_word_map.append(word_split[2])
    #         else:
    #             bad_word_map.append("[redacted]")
            
    #         bad_word_to_alternatives.append(bad_word_map)
    ...
    replacement = check_bad_words(message_input)
        ...
        with open('z_ServerConfig.txt', 'r') as file:
            ...
            for line in file_contents:
                if "modlog:" in line:
                    ...
                    embed_log.set_footer(text=f"Censored words: {bad_words}")
                    await log_channel.send(embed=embed_log)
                    break
